...
We will broadly define any "-etic" unit as being an equalizer or initial object for some set of functions that must be defined.
...
Each "-emic" unit for is an analogous equalizer equalizer or initial object for some other set of functions that must be defined.
...
The morphemes that indicate a "subject" in a Nominative-Accusative language correspond to sememes that refer to the semes of both the 
...
This distinction between the meaning of something and its representation(s) in one or more languages may be useful,
...
Such a concept could not possibly have sound associated to it, since the morph is by definition the smallest unit of sound that carries any meaning,
...
The tagmeme is also sometimes referred to as the "grammeme", however "tagmeme" is the term that literature has always recommended for usage.
We find Bloomfield's definition compatible with ours, however we provide our own definition here for clarity:
a "tagmeme" is *indivisible unit of distinction between any set of morphemes or their arrangement* in a given language.
...
The distinction between tags and episemes (or for that matter, tagmemes and episememes) is subtle,
...
To illustrate, we have stated that "singular" and "plural" are tags within Spanish tagmemics. 
